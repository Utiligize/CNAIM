...
function Chart() {
	...
	var main = function () {
		...
		//Add svg
		var svg = container
			.patternify({ tag: 'svg', selector: 'svg-chart-container_c' })
			.attr('id', 'custom-sized-matrix_c')
			...
			.attr('overflow', 'visible');
					 ...
					 d3.select('#htmlwidget-5').remove();
					 d3.select('#htmlwidget-6').remove();
		...
		const data0 = attrs.matrixData.filter(c=>c.x!='na' || c.y !='na')
		...
		if (attrs.intervalsSubmited) {
			...
			const data_x_intervals = attrs.matrixData.filter(c=>c.x_intervals!='na')
      const data_y_intervals = attrs.matrixData.filter(c=>c.y_intervals!='na')
    ...
    var total_width_data = calc.defaultRectWidth * groupedByX.length
      ...
      var cc2 = 0;
		...
		function addPointCircles(attrs, calc) {
			...
			const data1 = attrs.matrixData.filter(c=>c.point_x!='na' || c.point_y !='na')
			...
			var pointCircles = pointsContainer
			  ...
			  .attr('r', data1[0]['dot_radius'])
				...
				.on('mouseenter', function (d) {
					...
					var tooltipText = `Transformer TR243 in critical condition`; //show coordinates of the circle in tooltip
