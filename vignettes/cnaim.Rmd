...
```{r real-calculation, message=FALSE, warning=FALSE, include=FALSE}
# Generate an empty 5x4 matrix
matrix_structure <- risk_matrix_structure(5,4,NA)
# Monetary risk for one asset
risk_coordinates <- risk_calculation(matrix_dimensions = matrix_structure,
                                     ...
                                     cof = 18232,
                                     asset_type = "6.6/11kV Transformer (GM)")
adj_mat_1  = matrix_adjusted_circles(matrix_structure,
                                     risk_coordinates,
                                     dot_radius = 4)
```
...
r2d3(
  data = adj_mat_1,
  script = system.file("extdata/matrix_upper_level_circles.js" , package = 'CNAIM'),
  dependencies = system.file("extdata/matrix_d3script_circles_aa.js", package = 'CNAIM') ,
  container = 'div' 
)
```
<DIV id="d3-chart1">
...
```{r real-calculation-2, message=FALSE, warning=FALSE, include=FALSE}
# Non-linear bins -------
risk_data_matrix <- CNAIM:::example_risk_matrix
...
adj_mat_2 = CNAIM::matrix_adjusted_intervals(risk_data_matrix,x_intervals,y_intervals) 
```
```{r echo=FALSE} 
r2d3( 
  data = toJSON(adj_mat_2), 
  script = system.file("extdata/matrix_upper_level_nonlinear.js", package = 'CNAIM'),
  dependencies = system.file("extdata/matrix_d3script_chart_2.js", package = 'CNAIM'),
  container = "div"
)
```
<DIV id="d3-chart2">
...
```{r real-calculation-3, message=FALSE, warning=FALSE, include=FALSE}
# Non-linear bins -------
risk_data_matrix <- readRDS(system.file("extdata/risk_data_matrix.rds", package = "CNAIM"))
risk_data_matrix$value <- sample(1:30,size=nrow(matrix_structure),replace = T)
x_intervals = c(0.1,0.1,0.1,0.2,0.3)
x_intervals = x_intervals/sum(x_intervals) 
x_intervals = round(x_intervals,2) 
...
y_intervals = y_intervals/sum(y_intervals) 
y_intervals = round(y_intervals,2)
adj_mat_3 = matrix_adjusted_intervals(risk_data_matrix,x_intervals, y_intervals) 
```
<DIV id="d3-chart3">
```{r echo=FALSE} 
r2d3( 
  data = toJSON(adj_mat_3), 
  script = system.file("extdata/matrix_upper_level_nonlinear_bins.js", package = 'CNAIM'),
  dependencies = system.file("extdata/matrix_d3script_chart_3.js", package = 'CNAIM'),
  container = "div"
)
```
</DIV>
...
```{r real-calculation-4, message=FALSE, warning=FALSE, include=FALSE}
# Non-linear bins -------
risk_data_matrix <- CNAIM::risk_matrix_structure(4,4,NA)
risk_data_matrix$value <- sample(1:30,size=nrow(risk_data_matrix),replace = T)
x_intervals = rep(25,4)
x_intervals = x_intervals/sum(x_intervals) 
x_intervals = round(x_intervals,2) 
y_intervals = rep(25,4)
y_intervals = y_intervals/sum(y_intervals)
y_intervals = round(y_intervals,2)
adj_mat_4 = matrix_adjusted_intervals(risk_data_matrix,x_intervals, y_intervals) 
```
<DIV id="d3-chart4">
```{r echo=FALSE} 
r2d3( 
  data = toJSON(adj_mat_4), 
  script = system.file("extdata/matrix_upper_level_nonlinear_different_dimensions.js", package = 'CNAIM'),
  dependencies = system.file("extdata/matrix_d3script_chart_4.js", package = 'CNAIM'),
  container = "div"
)
```
</DIV>
...
```{r eval=T, message=FALSE, warning=FALSE, include=F}
risk_data_matrix = read.csv(system.file("extdata/matrix_data_new_values.csv", package = "CNAIM"), stringsAsFactors = F)
#random points
point_x = sample(1:100, 10)
point_y = sample(1:100, 10)
dots_vector = data.frame(id = 1:length(point_x), point_x , point_y)
dot_radius = 3
# x_intervals = sample(20:100,5) #random numbers
x_intervals = 50*c(0.01,0.01,0.01,0.02,0.03)
x_intervals = x_intervals/sum(x_intervals)
x_intervals = round(x_intervals,2)
y_intervals = 50*c(0.75,0.75,1,1.5)
# y_intervals = sample(20:100,4) #random numbers
y_intervals = y_intervals/sum(y_intervals)
y_intervals = round(y_intervals,2)
combined_matrix = matrix_combined(risk_data_matrix, dots_vector,
                                dot_radius, x_intervals, y_intervals)
r2d3(
  data = combined_matrix,
  script = system.file("extdata/matrix_upper_combined.js", package = 'CNAIM'),
  dependencies = system.file("extdata/matrix_d3script_combined.js", package = 'CNAIM'),
  container = "div"
)
```
</DIV>
